...
public class SubscriptionRepository {
    ...
    public Optional<Subscription> findActiveByIdForUpdate(UUID id) {
        List<Subscription> subscriptions = jdbcTemplate.query(
                """
                select id, member_id, plan_id, start_date, end_date, status::text as status, created_at
                from subscriptions
    ...
    public Optional<Subscription> findActiveByIdAndUserEmailForUpdate(UUID subscriptionId, String email) {
        List<Subscription> subscriptions = jdbcTemplate.query(
                """
                select s.id, s.member_id, s.plan_id, s.start_date, s.end_date, s.status::text as status, s.created_at
                from subscriptions s
